...
const TimeSeries = () => {
  ...
  const handleFileUpload = (event) => {
    ...
    Papa.parse(file, {
      ...
      complete: (results) => {
        ...
        // Detect type: if first column values parse as dates → timeseries, else data_table
        const firstVal = data[0] ? String(data[0][columns[0]]) : '';
        const looksLikeDate = firstVal && !isNaN(Date.parse(firstVal));
        const tsType = looksLikeDate ? 'timeseries' : 'data_table';
        ...
        const newTimeSeries = {
          ...
          columns: columns,
          type: tsType,
          dateColumn: looksLikeDate ? columns[0] : null,
          dataColumns: looksLikeDate ? columns.slice(1) : columns,
          rowCount: data.length,
          statistics: statistics,
          modelId: currentModel?.id || null,
          modelName: currentModel?.name || 'No Model',
          source: 'uploaded',
          // Data table config (for Calliope 0.7 data_tables)
          dataTableConfig: tsType === 'data_table' ? { rows: columns[0], columns: '', add_dims: {} } : null,
        };
  ...
  // Update the dataTableConfig for a data_table entry
  const updateDataTableConfig = (id, patch) => {
    setTimeSeries(timeSeries.map(ts => {
      if (ts.id !== id) return ts;
      const cfg = { ...(ts.dataTableConfig || {}), ...patch };
      const updated = { ...ts, dataTableConfig: cfg };
      if (selectedTimeSeries?.id === id) setSelectedTimeSeries(updated);
      return updated;
    }));
  };

  // Add a key to add_dims for the selected data table
  const addDim = (id) => {
    const ts = timeSeries.find(t => t.id === id);
    if (!ts) return;
    const cfg = ts.dataTableConfig || {};
    const add_dims = { ...(cfg.add_dims || {}), '': '' };
              ...
              <div className="flex-1 flex items-center gap-2 overflow-x-auto">
                {modelTimeSeries.map(ts => (
                  ...
                  >
                    ...
                    {ts.type === 'data_table' && (
                      <span className="px-1.5 py-0.5 text-[9px] font-semibold bg-indigo-100 text-indigo-700 rounded">DT</span>
                    )}
                    {ts.source === 'template' && (
        ...
        <div className="flex-1 overflow-hidden">
          {!selectedTimeSeries ? (
            ...
            <div className="h-full flex flex-col">
              ...
              {/* Chart and Stats Side by Side — only for timeseries type */}
              {selectedTimeSeries.type !== 'data_table' && <div className="flex-1 flex overflow-hidden">
