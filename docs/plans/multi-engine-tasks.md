...
| T19 | partial | 2026-07-17. Docs updated (import-export.md, running-optimization.md, CLAUDE.md). Cross-engine verification (docker exec) Calliope 0.6 vs 0.7: obj=541.4126 (0.0000% diff), all 12 frozen-contract keys present, 168 ts, caps match to 1e-13. Service /health OK on :5000, :5002, :5001. npm test: 42 passed. pytest: 128 passed, 3 skipped. Shadow price fix (2026-07-17): calliope07_runner._extract_shadow_prices rewritten — calls verbose_strings() to populate calliope_coords on ObjConstraint objects, reverse-lookups via bySymbol, normalises timestamps to '%Y-%m-%d %H:%M'. Verified in Docker: electricity:north + electricity:south, 168 ts, correct non-zero duals at congested hours. Remaining blockers (require in-app venv installs): PyPSA/OSeMOSYS end-to-end, AdOpT-NET0 (needs purpose-built reference fixture). Manual UI checklist (install flows, run/cancel/error per engine, Results/map render per engine) pending — requires running TEMPO app. |
