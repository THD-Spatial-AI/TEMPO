...
| T19 | partial | 2026-07-13. Docs updated: import-export.md rewritten for multi-engine (archive import, scenario datasets, ZIP structures per format); running-optimization.md rewritten for multi-engine (engine table, pre-run checklist, scenario runs, troubleshooting per engine). CLAUDE.md: process/port table extended with AdOpT-NET0/PyPSA/OSeMOSYS rows; Key Files table extended with 11 new entries. Cross-engine verification run (via docker exec) on Calliope 0.6.8 and 0.7 containers: both reach optimal, obj=541.4126 (0.0000% diff), all contract keys present, 168 timesteps, key-set identical — PASS. AdOpT-NET0: reference_model.json incompatible with adoptnet0 fixture expectations (needs electricity_network/ dir in period1/) — expected; would need a purpose-built AdOpT-NET0 reference fixture. PyPSA/OSeMOSYS: no Docker containers on this machine (local venv install required from Settings). Manual UI checklist and PyPSA/OSeMOSYS end-to-end pending. |
